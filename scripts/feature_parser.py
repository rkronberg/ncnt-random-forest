...
def main():
    ...
    for d in tqdm(dirs):
        ...
        str2 = "grep 'ENERGY|' %s/refs/%s/ncnt-geoopt.out | tail -1 | awk '{print $9}'" \
            % (CP2K_OUT_PATH,d)
        refEner = float(subprocess.check_output(str2,shell=True))

        # Unoptimized reference
        ref = read('%s/refs/%s/ncnt.xyz' % (CP2K_OUT_PATH,d))
        cell = ref.get_cell()

        # Optimized reference
        refopt = read('%s/refs/%s/ncnt-geoopt-pos.xyz' % (CP2K_OUT_PATH,d))
        ...
        for c in tqdm(range(dirs[d]+1),leave=False):
            str3 = "grep 'ENERGY|' %s/adsorbed/%s/ncnt_%s-geoopt.out | tail -1 | awk '{print $9}'" \
                % (CP2K_OUT_PATH,d,c)
            ...
            xyz = read('%s/adsorbed/%s/ncnt_%s-geoopt-pos.xyz' % (CP2K_OUT_PATH,d,c))
            xyz.set_cell(cell)
            ...
            str4 = "grep ' %s       %s' %s/refs/%s/ncnt-geoopt.out | tail -1 | awk '{print $8}'" \
                % (site+1,xyz.symbols[site], CP2K_OUT_PATH, d)
            nac = float(subprocess.check_output(str4,shell=True))
            q.append(nac)

            # Spin moment on adsorption site
            str5 = "grep ' %s       %s' %s/refs/%s/ncnt-geoopt.out | tail -1 | awk '{print $7}'" \
                % (site+1,xyz.symbols[site], CP2K_OUT_PATH, d)
            spin = float(subprocess.check_output(str5,shell=True))
            mu.append(spin)

            # Band gap
            str6 = "grep 'LUMO gap' %s/refs/%s/ncnt-geoopt.out | tail -2 | awk '{print $7}' | sort -n | head -1" \
                % (CP2K_OUT_PATH,d)
