...
def main():
    ...
    print(
        "Launching script for parsing features from CP2K output and .xyz coordinate files..."
    )
    print(
        "Implementation is very ad hoc, proceed with caution and check your results!\n"
    )
    print("Processing...")
    ...
    for d in dirs:
        ...
        str2 = (
            "grep 'ENERGY|' ../refs/%s/ncnt-geoopt.out | tail -1 | awk '{print $9}'" % d
        )
        ...
        for c in range(dirs[d] + 1):
            str3 = (
                "grep 'ENERGY|' ../adsorbed/%s/ncnt_%s-geoopt.out | tail -1 | awk '{print $9}'"
                % (d, c)
            )
            ...
            nearNNN = nlref[1][
                np.where(nlref[0] == nearN)
            ]  # Nearest neighbors of closest N
            ...
            if len(hydro) != 0:
                ...
                nearH = occupied[np.where(dHS == np.amin(dHS))][
                    0
                ]  # Occupied site closest to ads. site
            else:
                nearH = np.nan
            ...
            str4 = (
                "grep ' %s       %s' ../refs/%s/ncnt-geoopt.out | tail -1 | awk '{print $8}'"
                % (site + 1, xyz.symbols[site], d)
            )
            ...
            str5 = (
                "grep ' %s       %s' ../refs/%s/ncnt-geoopt.out | tail -1 | awk '{print $7}'"
                % (site + 1, xyz.symbols[site], d)
            )
            ...
            # Band gap
            str6 = (
                "grep 'LUMO gap' ../refs/%s/ncnt-geoopt.out | tail -2 | awk '{print $7}' | sort -n | head -1"
                % d
            )
            ...
            cV.append((224 - len(ref) + len(hydro)) / 224.0 * 100)
            ...
            rmsd.append(
                np.sqrt(
                    np.mean((xyz.get_positions()[:-1] - refopt.get_positions()) ** 2)
                )
            )
            rmaxsd.append(
                np.sqrt(
                    np.amax((xyz.get_positions()[:-1] - refopt.get_positions()) ** 2)
                )
            )
            ...
            while S < len(siteNNcycl):
                angle.append(
                    refopt.get_angle(siteNNcycl[S - 1], site, siteNNcycl[S], mic=True)
                )
                S += 1
            ...
            while S < len(NNNcycl):
                angle.append(
                    refopt.get_angle(NNNcycl[S - 1], nearN, NNNcycl[S], mic=True)
                )
                S += 1
