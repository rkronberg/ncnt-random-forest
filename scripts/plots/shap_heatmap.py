...
def plot(instance_order=Explanation.hclust(), feature_values=Explanation.abs.mean(0), 
            feature_order=None, max_display=15, cmap=colors.red_white_blue):
    ...
    instance_order = np.argsort(shap_values.sum(1))
    ...
    feature_names = np.array(featureSym)[feature_order]
    ...
    vmin = np.nanpercentile(values.flatten(), 2)
    vmax = np.nanpercentile(values.flatten(), 98)
    plt.imshow(
    ...
    fx = values.T.sum(0)
    plt.yticks([-fx[0]-4] + list(yticks_pos), [r"$\hat{f}(\mathbf{x})$"] + list(yticks_labels), fontsize=16)
    
    plt.ylim(values.shape[1]-0.5, -5)
    
    plt.text(values.shape[0]+values.shape[0]/50,-4,r'$\mathbb{E}[\hat{f}(\mathbf{x})]$',ha='left',va='center', fontsize=16)
    ...
    plt.axhline(-4, color="k", alpha=0.5, linestyle="--", linewidth=1.5)
    plt.plot(-fx-4, color="k", lw=1.5)
    #plt.colorbar()
    ...
    b = plt.barh(
        ...
        0.7, align='center', color="#000000", left=values.shape[0] * 1.0 + 10
        #color=[colors.red_rgb if shap_values[feature_inds[i]] > 0 else colors.blue_rgb for i in range(len(y_pos))]
    )
    for v in b:
        v.set_clip_on(False)
    plt.xlim(-10, values.shape[0]+10)
    plt.xlabel(r'Configuration')
    plt.tick_params(which='both',direction='in',left=False)
    
    
    
    if True:
        import matplotlib.cm as cm
        cax = fig.add_axes([.84,.127,.04,.523])
        m = cm.ScalarMappable(cmap=cmap)
        m.set_array([min(vmin,-vmax), max(-vmin,vmax)])
        cb = plt.colorbar(m, ticks=[min(vmin,-vmax), max(-vmin,vmax)], aspect=1000, cax=cax)
        cb.set_ticklabels(['Low', 'High'])
        cb.set_label(r"$\phi_j$", labelpad=-10)
    ...
    plt.savefig('%s/heatmap.pdf' % DATA_PATH)