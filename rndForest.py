...
def plotSHAP(shap_i, shap_c, featureNames):
    ...
    for i in ind_shap[-10:]:
        if corr_ave[i] >= 0:
            ...
            plt.barh(
                featureSym[i], shap_ave[i], xerr=shap_std[i], capsize=4, color="C3"
            )
        else:
            print("%s: %s (-)" % (featureNames[i], shap_ave[i]))
            plt.barh(
                featureSym[i], shap_ave[i], xerr=shap_std[i], capsize=4, color="C9"
            )
...
def crossval(x, y, model, strat):
    ...
    print(
        "%s-fold cross-validation (training data: %s, test data: %s)"
        % (n_splits, len(train), len(test))
    )
    print(
        "R2 score (Training set): %.4f +- %.4f" % (score_R2_train, score_R2_train_std)
    )
...
def rndsearch(x, y, strat):
    ...
    dists = dict(n_estimators=np.arange(500, 600, 100), max_features=np.arange(8, 13))

    x_train, x_test, y_train, y_test = train_test_split(
        x, y, test_size=0.1, stratify=strat, shuffle=True, random_state=rnd
    )
    rf = RandomizedSearchCV(
        estimator, dists, n_iter=20, cv=10, random_state=rnd, n_jobs=-1
    )
...
def plot(
    ...
    if LC:
        ...
        ax2.minorticks_on()
        ax2.tick_params(
            which="both", direction="in", top=True, right=True, labelsize=14
        )
...
def main():
    ...
    rf = RandomForestRegressor(
        n_estimators=500, max_features=10, oob_score=True, random_state=rnd, n_jobs=-1
    )

    # Cross-validation
    y_train, y_pred_train, y_test, y_pred_test, shap_i, shap_c = crossval(
        x, y, rf, strat
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-p", "--plot", action="store_true", help="Plot predictions vs. DFT data"
    )
    parser.add_argument(
        "-l", "--lcurve", action="store_true", help="Plot learning curve"
    )
    parser.add_argument(
        "-r",
        "--rndsearch",
        action="store_true",
        help="Do randomized hyperparameter search",
    )
